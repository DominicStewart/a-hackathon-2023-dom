  ...
  (function () {
    ...
    function _writeBatch() {
      _writeBatch = _asyncToGenerator( /*#__PURE__*/_regeneratorRuntime().mark(function _callee2(batchCollection) {
        var _ref2;
        var aggregatedReports, filteredReports, _iterator3, _step3, report, key, filteredReport, _iterator4, _step4, prop, averagedReports;
        return _regeneratorRuntime().wrap(function _callee2$(_context2) {
          while (1) switch (_context2.prev = _context2.next) {
            case 0:
              aggregatedReports = (_ref2 = []).concat.apply(_ref2, _toConsumableArray(batchCollection));
              filteredReports = [];
              _iterator3 = _createForOfIteratorHelper(aggregatedReports);
              try {
                for (_iterator3.s(); !(_step3 = _iterator3.n()).done;) {
                  report = _step3.value;
                  for (key in report) {
                    if (key !== "connectionId" && key !== "reportNum") {
                      if (_filter.types.includes(report[key].type) && _filter.kinds.includes(report[key].kind)) {
                        filteredReport = {};
                        _iterator4 = _createForOfIteratorHelper(_filter.props);
                        try {
            ...
            case 6:
              ...
              if (movingWindow.length > _config.avgInterval) {
                averagedReports = calculateAverages();
    ...
    function calculateAverages() {
      var sums = {},
        counts = {},
        results = [];
      var nonAveragedProps = new Set(['id', 'type', 'kind', 'mid']); // Add any property names that should not be averaged
      var _iterator = _createForOfIteratorHelper(movingWindow),
        _step;
      try {
        for (_iterator.s(); !(_step = _iterator.n()).done;) {
          var report = _step.value;
          var _iterator2 = _createForOfIteratorHelper(report),
            _step2;
          try {
            for (_iterator2.s(); !(_step2 = _iterator2.n()).done;) {
              var item = _step2.value;
              var _itemId = item.id + '|' + item.type;
              if (!sums[_itemId]) {
                sums[_itemId] = {};
                counts[_itemId] = {};
              }
              for (var _key in item) {
                if (!counts[_itemId][_key]) {
                  counts[_itemId][_key] = 0;
                  sums[_itemId][_key] = 0;
                }
                if (nonAveragedProps.has(_key)) {
                  // For non-averaged properties, we take the first occurrence
                  if (counts[_itemId][_key] === 0) {
                    sums[_itemId][_key] = item[_key];
                    counts[_itemId][_key] = 1; // Prevent this property from being overwritten
                  }
                } else {
                  if (typeof item[_key] === 'number') {
                    sums[_itemId][_key] += item[_key];
                    counts[_itemId][_key]++;
                  }
                }
                if (isNaN(sums[_itemId][_key])) {
      ...
      for (var itemId in sums) {
        var id_type = itemId.split('|');
        var itemResult = {
          id: id_type[0],
          type: id_type[1]
        };
        for (var key in sums[itemId]) {
